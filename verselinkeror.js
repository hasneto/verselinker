!async function() {
    ...
    try {
        ...
        async function v(e) {
            ...
            r.className = "bibliatodo-tooltip", r.style.position = "absolute", r.style.backgroundColor = "#fff", r.style.border = "1px solid #ccc", r.style.zIndex = "99999999", r.style.boxShadow = "0px 4px 6px rgba(0, 0, 0, 0.1)", r.style.fontFamily = "Arial, sans-serif", r.style.fontSize = "14px", r.style.maxWidth = "380px", r.style.borderRadius = "7px", r.innerHTML = '<div style="padding: 7px; text-align: center;color: black!important;">Loading...</div>', document.body.appendChild(r), s = r, d.push(r);
        ...
        function E(e) {
            ...
            const r = Object.keys(f).map((e => e.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"))).join("|"),
                a = new RegExp(`\\b(${r}|judas|jd|obadias|ob|filemom|fm|2\\s*joão|2joão|2jo|3\\s*joão|3joão|3jo})\\s+(\\d+)(?::(\\d+(?:-\\d+)?(?:,\\d+(?:-\\d+)?)*))?\\s*(\\(([A-Za-z0-9-]{1,20})\\))?`, "gi");
            let l = n.replace(a, ((e, t, o, n, r, a, l, s) => {
                if ("%".charAt(l + e.length)) return e;
                const c = t.trim().toLowerCase(),
                    d = f[c];
                if (!d) return e;
                if (x(o, d.cant_capitulos)) return e;
                let p = `${d.id}.1`; // Considerando que todos têm apenas um capítulo
