!async function(){
    ...
    let t = e.getAttribute("lang") || null,
        o = e.getAttribute("data-trueTooltip") || true,
        n = e.getAttribute("data-trueCredit") || true,
        r = e.getAttribute("data-trueLinks") || true;
    ...
    try {
        ...
        async function v(e) {
            ...
            r.style.borderRadius = "7px 7px 0 0";
            ...
            c = setTimeout(async () => {
                try {
                    ...
                    u += '<div style="padding: 7px; line-height: 23px;color: black!important;">';
                    u += s.data.map((e) => `<span><sup>${e.num_versiculo}</sup> ${e.info_versiculo}</span>`).join(" ");
                    if (0 === s.complete) {
                        u += `<div style="margin-top: 10px; text-align: left;">
                                <a href="${e.target.href}" target="_blank" style="color: #007bff!important; text-decoration: none; font-size: 12px;">More »</a>
                            </div>`;
                    }
                    u += "</div>";
                    ...
                    r.addEventListener("mouseleave", (() => { r.remove(); const e = d.indexOf(r); -1 !== e && d.splice(e, 1); }));
                } catch (e) {
        ...
        function E(e) {
            ...
            const a = new RegExp(`\\b(${r})\\s+(\\d+(?:-\\d+)?(?:,\\d+(?:-\\d+)?)*)(?::(\\d+(?:-\\d+)?(?:,\\d+(?:-\\d+)?)*))?\\s*(\\(([A-Za-z0-9-]{1,20})\\))?`, "gi");
            let l = n.replace(a, ((e, t, o, n, r, a, l, s) => {
                const c = t.trim().toLowerCase(), d = f[c];
                if (!d) return e;
                if (x(o, d.cant_capitulos)) return e;
                let p = `${d.id}.${o}`;
                n && (p += `:${n}`);
                r && (p += ` (${r})`);
                a && (p += `<span class="verse_tooltip_credits"> [${a}]`);
                return p;
            }));
            return l;
        }
